...
from simple3D.components.mouseMoveAround import MouseMove
...
class Scene:
    ...
    def render(self):
        ...
        if self.camera is None:
            self.camera = Camera(self.width/2, self.height)
        ...
        while not glfw.window_should_close(self.window):
            ...
            if now - previous >= time:
                ...
                glViewport(0, 0, int(self.width*0.5), int(self.height))
                self.camera.render(self.meshObjs)
                glViewport(int(self.width*0.5), 0, int(self.width*0.5), self.height)
                self.camera.render(self.meshObjs)
